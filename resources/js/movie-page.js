    ...
    fetch("../php/getMovies.php").then(response => response.json()).then(data => {
        const selectedMovie = data.find(movie => movie.title === selectedMovieTitle);
         if (selectedMovie){
            ...
            const edit_movie_anchor = document.getElementById("edit-movie-anchor");
            if (targetedUsername === selectedMovie.creator) {
                const edit_movie_btn = document.createElement("button");
                edit_movie_btn.className = "edit-movie-btn";
                edit_movie_btn.textContent = "Bearbeiten";
                edit_movie_anchor.appendChild(edit_movie_btn);

                edit_movie_anchor.href = `../php/edit_film.php?id=${selectedMovie.id}`;
            }
